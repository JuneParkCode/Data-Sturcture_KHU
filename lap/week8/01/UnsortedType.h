...
public:
  ...
  NodeType<ItemType> *GetData() const;
...
template <class ItemType>
ItemType SumSquare(NodeType<ItemType> *lst)
{
  if (lst == NULL)
    return (0);
  return (SQUARE(lst->info) + SumSquare(lst->next));
}

template <class ItemType>
void UnsortedType<ItemType>::PrintSumSquare() const
{
  std::cout << SumSquare(this->listData);
}

template <class ItemType>
NodeType<ItemType> *UnsortedType<ItemType>::GetData() const
{
