...
public:
	...
	bool DepthFirstSearch(VertexType, VertexType) const;
	//  void ClearMarks();
...
/*
	EX03
	BASE CASE
		startVertex is endVertex -> end recursion
	GENERAL CASE
		인접 노드에 대해서 DFS 실행
*/
template <class VertexType>
bool GraphType<VertexType>::DepthFirstSearch(VertexType startVertex, VertexType endVertex) const
{
	...
	int idxOfStartVertex = IndexIs(vertices, startVertex);
	for (int vertexIdx = 0; vertexIdx < numVertices; ++vertexIdx)
	{
		if (edges[idxOfStartVertex][vertexIdx] != NULL_EDGE)
		{
			vertexQueue.Enqueue(vertices[vertexIdx]);
		}
	}
